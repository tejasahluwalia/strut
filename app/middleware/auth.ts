...
export async function verifySession(value: { sessionId: string } | null) {
  if (!value || !value.sessionId) return null;

  let sessionRecord = await db.findOne(sessionTable, {
  ...
  // Return the user
  let userRecord = await db.findOne(userTable, {
...
export function requireAdminAuth() {
  return requireAuth({
    onFailure(context) {
      ...
      return redirect(routes.admin.auth.login.href() + `?returnTo=${returnTo}`, 302);
    },
  });
}
