...
- Configuring `createAssetServer` (`basePath`, `fileMap`, `allow`, `deny`,
...
- `fileMap` keys are URL patterns relative to `basePath`, and values are
  root-relative file path patterns. They use `route-pattern` syntax on both
  sides.
