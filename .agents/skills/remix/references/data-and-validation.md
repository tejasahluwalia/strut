...
- `validate` runs before `create` and `update` writes and should return either
  `{ value }` or `{ issues }`
...
// Query builder
let genres = await db.query(books).select("genre").distinct().orderBy(
  "genre",
  "asc",
).all();
...
import { authAccounts, users } from "../../app/data/schema.ts";
